  ...
  observeEvent(
    ...
    handlerExpr = {
      output$randomForest <- renderPrint({
        ...
        set.seed(42)
        x <- input$class
        model <-  randomForest(as.factor(df[indices.train,x]) ~ ., data=df[indices.train ,c(input$predictors, input$class)],mtry = as.integer(input$mtry), ntree = as.integer(input$ntree), nodesize=as.integer(input$nodesize))
        model
      })
    }
  )
